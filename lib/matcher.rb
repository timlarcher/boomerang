...
def self.do_payoff! ( from_index )
  ...
  for i in from_index..number_of_invoices
    puts "do_payoff: #{i}"
    @invoices_stack[i].amount -= payoff_amount
    if @invoices_stack[i].amount == 0.0
      @invoices_stack[i].delete
...
def self.do_match ( client_id )
  ...
  lid = client_id
  current_query = Boomerang::Matching.where( :payee_client_id => lid )
  ...
  loop do
    ...
    lowest_amount = current_invoice.amount if current_invoice.amount < lowest_amount or lowest_amount == 0.0
